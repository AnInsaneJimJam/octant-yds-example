...
/// @dev Works with Morpho adapter to demonstrate real yield generation and donation.
contract DemoDonate is Script {
    function run() external {
        ...
        // With Morpho adapter, yield is generated naturally over time through lending
        // For demo purposes, we can simulate time passage or just harvest current yield
        // (We intentionally ignore return/revert via tryCall for backward compatibility.)
        Helpers.tryCall(vaultAddr, abi.encodeWithSignature("bumpYield(uint256)", depositAmount / 10)); // +10% (mock only)
